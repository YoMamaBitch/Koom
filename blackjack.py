...
class BlackjackGame():
    ...
    async def playerLose(self):
        ...
        self.bot.db.koomdata.update_one({'_id':ObjectId(secrets.lotteryID)}, {'$inc':{'_lotteryAmount':float(self.amount * secrets.tax)}})
    ...
    async def playerBust(self):
        ...
        newMoney = userObject['_currency'] - float(self.amount)
        self.bot.db.koomdata.update_one({'_uid':self.player}, {'$set':{'_currency':newMoney}})
        self.bot.db.koomdata.update_one({'_id':ObjectId(secrets.lotteryID)}, {'$inc':{'_lotteryAmount':int(self.amount * secrets.tax)}})
